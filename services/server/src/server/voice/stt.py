...
(`agent/backend/providers/base.py`). `GroqSTTProvider` woła Groq (Whisper przez ich
API, `AsyncGroq`) — kontrakt zweryfikowany bezpośrednio (dokumentacja + inspekcja
zainstalowanego SDK, nie zgadywany). `MockSTTProvider` pozostaje dev-providerem
(fallback gdy `VoiceProvidersConfig.groq_api_key` puste, patrz `main.py`).
"""
