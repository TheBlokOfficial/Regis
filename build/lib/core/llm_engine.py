...
class LLMEngine:
    ...
    def generate_response(self, messages: list[dict], tools_registry, on_tool_call: Any = None, on_thought_token: Any = None, on_content_token: Any = None) -> str:
        ...
        if self.tier == "butler":
            ...
            return agent.generate_response(messages, tools_registry, on_tool_call, on_thought_token, on_content_token)
        else:
            agent = ReActAgent(self.model_name, self.temperature)
            return agent.generate_response(messages, tools_registry, parser, on_tool_call, on_thought_token, on_content_token)
