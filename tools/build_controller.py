...
def deploy_to_pi():
    ...
    ssh_cmd = [
        ...
        f"sudo bash -c 'cat > /etc/systemd/system/regis-stt.service <<EOF\n"
        f"[Unit]\n"
        f"Description=Regis STT Worker\n"
        f"After=network.target\n\n"
        f"[Service]\n"
        f"User={server.split('@')[0]}\n"
        f"WorkingDirectory={remote_dir}\n"
        f"ExecStart={remote_dir}/.venv/bin/python -m node.stt_worker\n"
        f"Restart=always\n\n"
        f"[Install]\n"
        f"WantedBy=multi-user.target\n"
        f"EOF' && "
        f"sudo systemctl daemon-reload && "
        f"sudo systemctl enable regis-stt.service && "
        f"sudo systemctl restart regis.service regis-worker.service regis-stt.service && "
        ...
        f"echo '\\n[Status Regis STT]' && systemctl status regis-stt.service --no-pager | grep Active"
    ]
