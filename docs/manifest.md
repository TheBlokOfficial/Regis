...
- **Sekcje kontekstu tury — komponowalna lista, nie stały zestaw slotów (2026-08-22)**: `world/prompt_sections.py` trzyma **uporządkowaną listę** sekcji (`data/world/prompt_sections.json`); kolejność listy = kolejność w prompcie. Każda sekcja ma tekst, warunek pojawienia się i opcjonalną negację. Użytkownik dodaje, usuwa i przestawia sekcje w Web UI (Świat → Kontekst tury).
  ...
  **Negacja zamiast bliźniaczych warunków**: `delivery_text` to ta sama sekcja co `delivery_voice`, tylko z `negated=True` — pierwszy realny dowód, że pole `negated` usuwa potrzebę mnożenia warunków.
