...
- **Bramka energii i VAD po stronie satelity, wake-word na serwerze**: W stanie nasłuchu satelita mierzy amplitudę każdej ramki, ale nie wysyła ciszy. Zachowuje około 1,7 s pre-rollu; po pojawieniu się dźwięku otwiera krótką porcję transmisji, najpierw każe serwerowi wyzerować bufor modelu (`wake_stream_start`), a następnie dosyła pre-roll i bieżące audio. Pre-roll jest celowo niemal tak długi jak 2-sekundowe okno `OnnxWakeWordDetector`: pierwsza pełna predykcja nadal obejmuje początek słowa. Dzięki temu bezczynny pokój nie uruchamia inferencji na Pi, a oddzielne porcje dźwięku nie sklejają się w sztuczny ciąg. Po wykryciu wake-worda lokalny VAD nadal sam podejmuje decyzję o końcu wypowiedzi (`vad_silence_duration_ms` ciszy), bo satelita musi wiedzieć, kiedy przestać nagrywać bez dodatkowego roundtripu.
