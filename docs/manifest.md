...
- **`OllamaProvider` / `OpenRouterProvider` / `GroqProvider` (`ai/llm/providers/`)**: Konkretne implementacje `BaseLLMProvider`. Wszystkie trzy wspierają tool calling. `GroqProvider` (dodany 2026-08-21) to trzeci OpenAI-compatible REST backend obok OpenRouter — endpoint `https://api.groq.com/openai/v1/chat/completions`, format SSE/tool_calls identyczny jak OpenRouter (kontrakt zweryfikowany w dokumentacji Groq, nie zgadywany), ale **bez** pola `reasoning`/nagłówków `HTTP-Referer`/`X-Title` — to rozszerzenia specyficzne dla OpenRouter, nieudokumentowane w API Groq. Domyślny model `llama-3.3-70b-versatile`.
...
- **`BackendRegistry` (`ai/llm/registry.py`)**: Dynamiczny rejestr dostawców modeli (pliki JSON w `data/backends/`) z możliwością płynnego przełączania aktywnego backendu (np. z lokalnego `OllamaProvider` na chmurowy `OpenRouterProvider`/`GroqProvider`).
