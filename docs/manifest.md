...
- **Sekcje kontekstu tury — tekst promptu jako konfiguracja, nie kod (2026-08-22)**: `world/prompt_sections.py` trzyma domyślne teksty (dawne literały) i nadpisania z Web UI (`data/world/prompt_sections.json`). Semantyka trójstanowa: `None` = domyślna, `""` = **wycisz sekcję całkowicie**, tekst = użyj go. Silnik nadal decyduje, KTÓRE sekcje się pojawią (tylko on wie, czy nadawca ma pokój i czy są urządzenia) — użytkownik decyduje, CO mówią.

  **Świadomie odrzucono szablon z warunkami** (Jinja2 w textarea): oddanie użytkownikowi edytora kodu bez walidacji oznacza, że literówka w składni wywala każdą turę, plus sandbox i nowa zależność. Odrzucono też jeden wielki szablon z placeholderami — dałby kontrolę nad kolejnością, ale **traci warunkowość**: bez przypisanego pokoju `{pokój}` byłoby puste i zostałoby kalekie "Nadawca znajduje się w lokalizacji: .". Ratowanie tego regułą "pusty placeholder kasuje akapit" byłoby magią, która zaskakuje.
