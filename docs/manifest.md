...
│   ├── engine.py     # WorldEngine — build(), CRUD configu/grup/deklaracji/nadawców, speak_in_room
│   ├── client.py     # HomeAssistantClient
│   ├── models.py     # Device, DeviceGroup, HomeAssistantConfig, SenderProfile
...
- **`WorldEngine` (`engine.py`)**: Konfiguracja Home Assistant (singleton, jeden `base_url`/`access_token`), zadeklarowana lista urządzeń (opt-in), grupy i przypisania nadawców do pokoi — wszystko jako pliki JSON pod `data/world/` (`config.json`, `declared_devices.json`, `groups/*.json`, `senders.json`). `build(sender_id, voice_mode)` czyta przypisanie nadawcy niezależnie od stanu Home Assistant (patrz sekcja 3 wyżej), segreguje urządzenia po `Device.area`, składa `dynamic_context` jako jeden string i zwraca `dispatch` wołający bezpośrednio `HomeAssistantToolExecutor`/logikę `get_time`/`speak_in_room` po natywnym `entity_id` — **bez pośredniej warstwy opaque ID**: skoro istnieje dokładnie jeden silnik, nie ma ryzyka kolizji identyfikatorów między wieloma źródłami, więc nie ma po co ich ukrywać. `voice_mode: bool` to **efemeryczny parametr wywołania** (dostarczany przez `server/voice`, nigdy trwały stan World) — steruje wyłącznie tym, czy `build()` dokleja zdanie "pisz krótko, to głos" do `dynamic_context`.
...
- **`Device` / `DeviceGroup` / `SenderProfile` (`models.py`)**: `Device.id` to wprost natywny `entity_id` Home Assistant (singleton — bez przestrzeni nazw połączenia). `Device.capabilities` to mapa nazwa narzędzia → granularne cechy (`dict[str, frozenset[str]]`). `Device.area` to natywny `area_id` Home Assistant — **jedyne** źródło pojęcia "pokój" w systemie, nadal nieobecne w kernelu (patrz sekcja 5). `SenderProfile` (wyłącznie `room_key`/`room_label`, **bez** kanału komunikacji ani tożsamości urządzenia — to wiedza `server/voice`, patrz sekcja 3.5) mapuje opaque `sender_id` na pokój — `room_key` musi zgadzać się z `Device.area`, co jest zależnością **konfiguracyjną** (administrator rejestrujący nadawcę), nie kodową; `GET /api/v1/world/areas` (unikalne `area_id` wśród zadeklarowanych urządzeń) istnieje wyłącznie jako wygoda formularza rejestracji.
...
- **Lokalizacja nadawcy to wewnętrzna wiedza `WorldEngine`, kanał komunikacji — nie**: Pierwsza wersja (`SatelliteRegistration.channel`) trzymała kanał (głos/tekst) jako trwały config World, analogicznie do `room_key`. **Zrewidowane** po analizie: to trwały config administrowany ręcznie mógł rozjechać się z rzeczywistością (satelita fizycznie łącząca się przez WS audio, ale zarejestrowana z `channel="text"` albo w ogóle niezarejestrowana) — a `server/voice` **strukturalnie zawsze wie z całą pewnością**, czy dana interakcja jest głosowa, bo to ona ją obsługuje. Kanał przestał więc być polem w `SenderProfile` — jest **efemerycznym parametrem wywołania** (`voice_mode: bool`, dostarczanym przez gateway do `WorldEngine.build()`, ten sam mechanizm co `tools_available: bool` w `ContextBuilder`), eliminując możliwość rozjazdu przez konstrukcję, nie przez dyscyplinę admina. Lokalizacja (`room_key`/`room_label`) zostaje trwałym configiem World — to nie ten sam rodzaj ryzyka (administrator i tak fizycznie umieszcza satelitę w pokoju w tym samym momencie, co ją rejestruje). `WorldEngine.build()` liczy framing lokalizacji **przed** próbą kontaktu z Home Assistant, więc padnięcie/brak konfiguracji HA ucina tylko listę urządzeń, nigdy framing lokalizacji/kanału.
...
- **Brak rdzennego pojęcia "pokoju" (`Room`) w kernelu**: Narzucałoby kernelowi założenie „świat = dom z pokojami”, podczas gdy smart home jest tylko jedną z możliwych domen agenta. `Device.area`/`SenderProfile.room_key` pozostają wyłącznie wewnętrznym słownictwem `WorldEngine` — kernel go nie zna.
...
5. **Zakładka ogólnej konfiguracji systemu**: Web UI ma dziś jeden, bezpośredni widok konfiguracji Home Assistant/nadawców (`web/js/views/extensions.js` montuje `HomeAssistantExtensionView` wprost, bez generycznej listy). Szersza, ogólna zakładka konfiguracji systemu jest wizją końcową, nieporuszaną przez ten refaktor.
