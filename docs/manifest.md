...
- **`connected_sender_ids`** (od tej sesji, `main.py`+`gateway.py`+`routes.py`): zwykły, współdzielony `set[str]` (bez locka — jeden wątek asyncio) wypełniany przez `voice_endpoint()` (`add` po `websocket.accept()`, `discard` w `finally`), czytany przez `GET /api/v1/voice/connected` (`routes.py`). Mechaniczny fakt "kto ma żywe połączenie WS teraz" — zero wiedzy o rejestracji/pokoju (to należy do `World`). Web UI (panel Nadawcy, Świat → `satellites_panel.js`) łączy tę listę z `World.senders.json` po stronie przeglądarki (nie serwera — `world`/`voice` nadal się nie importują), żeby pokazać satelity podłączone, ale jeszcze niezarejestrowane, z jednoklikowym skrótem do rejestracji.
