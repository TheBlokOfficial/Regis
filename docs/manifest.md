...
3. **Widoczność kroków ReAct w toku generowania (polling fallback)**: `startPolling` (Web UI, fallback gdy SSE nie jest aktywne — np. po odświeżeniu strony w trakcie długiej pętli ReAct) pokazuje tylko narastający tekst finalnej odpowiedzi, bez kroków pośrednich — `metadata.steps` istnieje dopiero po zakończeniu tury.
   ...
   **Powiązany bugfix (2026-08-21)**: `ChatView` widziała tury zainicjowane przez tę samą kartę przeglądarki (własne SSE) albo generowanie trwające już w momencie ładowania strony (`startPolling` wywołany z `loadSessionHistory`) — turę odpaloną **z zewnątrz** (satelita, cron, inna karta) obecnie otwarta karta widziała dopiero po ręcznym odświeżeniu, bo backend (`GET /api/v1/chat/sessions` z `is_generating`/`updated_at`, `GET .../history`) miał już wszystko potrzebne, ale nic po stronie klienta o to nie pytało w tle. Naprawione lekkim watchdogiem (`ChatView.startWatch`/`checkForExternalUpdates`, `chat.js`, co 3s, tylko gdy karta nie generuje własnej tury): porównuje listę sesji (wykrywa nowe) i `updated_at` aktywnej sesji (wykrywa nowe wiadomości/początek generowania), w razie zmiany przeładowuje historię — bez nowego kanału WS/EventBus do przeglądarki, czysto na istniejącym REST.
