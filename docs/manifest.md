...
- **`WorldEngine` (`engine.py`)**: Konfiguracja Home Assistant (singleton, jeden `base_url`/`access_token`), zadeklarowana lista urządzeń (opt-in), grupy, pokoje, przypisania nadawców do pokoi i profile promptu — wszystko jako pliki JSON pod `data/world/` (`config.json`, `declared_devices.json`, `groups/*.json`, `rooms/*.json`, `senders.json`, `prompts/*.json`+`active_prompt.json`). `build(sender_id, voice_mode)` pobiera treść aktywnego profilu promptu (`WorldPromptStore.get_active_content()`, może być pusty string — brak persony), doklejają ją PRZED faktami (przypisanie nadawcy niezależnie od stanu Home Assistant, patrz sekcja 3 wyżej; urządzenia posegregowane po `Device.room_id`), i składa całość jako jeden `system_prompt` string — **World jest jedynym autorem tej tury**, kernel niczego nie skleja. Zwraca też `dispatch` wołający bezpośrednio `HomeAssistantToolExecutor`/logikę `get_time`/`speak_in_room` po natywnym `entity_id` — **bez pośredniej warstwy opaque ID**: skoro istnieje dokładnie jeden silnik, nie ma ryzyka kolizji identyfikatorów między wieloma źródłami, więc nie ma po co ich ukrywać. `voice_mode: bool` to **efemeryczny parametr wywołania** (dostarczany przez `server/voice`, nigdy trwały stan World) — steruje wyłącznie tym, czy `build()` dokleja zdanie "pisz krótko, to głos" do `system_prompt`.
...
- **`EventBus` (`event_bus.py`)**: Asynchroniczna magistrala zdarzeń pub/sub (`subscribe`/`publish`). **W pełni wpięta w przepływ strumieniowania** — `AgentEngine` publikuje zdarzenia `ServerEventType.CHAT_CHUNK/DONE/ERROR/CANCELLED` oraz `TOOL_CALL_START/TOOL_CALL_RESULT` (kroki pętli ReAct), otagowane `session_id` (patrz sekcja 3.5, `effective_session_id`/`ToolResult.redirect_sender_id`), a `interact_stream` subskrybuje je i tłumaczy z powrotem na strumień ustrukturyzowanych `StreamEvent` (`agent/engine.py`) dla wywołującego. Dzięki temu rdzeń nie zna bezpośrednio odbiorców — dziś dwóch: SSE (HTTP, `routes/chat.py`, subskrypcja per-request) i WS satelit głosowych (`server/voice/gateway.py`, subskrypcja ciągła per-połączenie, patrz sekcja 3.5). `routes/chat.py` serializuje `StreamEvent` na ramki SSE z polem `type` (`chunk`/`tool_start`/`tool_result`). Ustrukturyzowany ślad kroków (`ToolStepPayload`: `call_id`/`name`/`text_offset`/`arguments`/`content`/`is_error`) trafia też — gdy tura użyła narzędzi — do `metadata.steps` finalnej wiadomości `assistant` w `MemoryManager`, więc Web UI potrafi odtworzyć całe drzewko ReAct (tekst/COT przeplecione z wywołaniami narzędzi) zarówno na żywo, jak i po powrocie do historii sesji.
...
- **Wake-word: realny model od tej sesji** (serwerowy `OnnxWakeWordDetector`, sekcja 3.6, `Settings.wakeword_model_path`) — **nadal placeholder**: STT/TTS (`MockSTTProvider`/`MockTTSProvider`) — klient desktopowy dowodzi poprawności całego protokołu i lokalnego VAD, ale nie podłącza jeszcze realnego rozpoznawania/syntezy mowy.
...
- **Lokalizacja nadawcy to wewnętrzna wiedza `WorldEngine`, kanał komunikacji — nie**: Pierwsza wersja (`SatelliteRegistration.channel`) trzymała kanał (głos/tekst) jako trwały config World, analogicznie do lokalizacji. **Zrewidowane** po analizie: to trwały config administrowany ręcznie mógł rozjechać się z rzeczywistością (satelita fizycznie łącząca się przez WS audio, ale zarejestrowana z `channel="text"` albo w ogóle niezarejestrowana) — a `server/voice` **strukturalnie zawsze wie z całą pewnością**, czy dana interakcja jest głosowa, bo to ona ją obsługuje. Kanał przestał więc być polem w `SenderProfile` — jest **efemerycznym parametrem wywołania** (`voice_mode: bool`, dostarczanym przez gateway do `WorldEngine.build()`, ten sam mechanizm co `tools_available: bool` w `ContextBuilder`), eliminując możliwość rozjazdu przez konstrukcję, nie przez dyscyplinę admina. Lokalizacja (`SenderProfile.room_id`) zostaje trwałym configiem World — to nie ten sam rodzaj ryzyka (administrator i tak fizycznie umieszcza satelitę w pokoju w tym samym momencie, co ją rejestruje). `WorldEngine.build()` liczy framing lokalizacji **przed** próbą kontaktu z Home Assistant, więc padnięcie/brak konfiguracji HA ucina tylko listę urządzeń, nigdy framing lokalizacji/kanału.
...
- **Brak rdzennego pojęcia "pokoju" (`Room`) w kernelu**: Narzucałoby kernelowi założenie „świat = dom z pokojami”, podczas gdy smart home jest tylko jedną z możliwych domen agenta. `Room` (patrz wyżej) jest pełnoprawnym bytem **World** — kernel wciąż go nie zna, zna wyłącznie mechaniczny `voice_mode`/opaque `sender_id`. `Device.room_id`/`SenderProfile.room_id` pozostają wyłącznie wewnętrznym słownictwem `WorldEngine`.
...
2. **Realny model wake-word — GOTOWE od tej sesji, chmurowy STT/TTS nadal planowany**: `server/voice` ma pełny szkielet end-to-end (`WS /ws/voice/{sender_id}`, automat stanu, protokół ramek — patrz sekcja 3.5) i od tej sesji **realny model wake-word** (`OnnxWakeWordDetector`, model `regis.onnx` wytrenowany w osobnym repo `Regis-Wakeword`, biblioteka `livekit-wakeword`, sekcja 3.6). Metryki treningowe (`regis_eval.json`): recall ~61%, FPPH ~0.2, próg optymalny 0.39 — działający, ale niedoskonały pierwszy model (multilingual TTS dla polskiej wymowy), do iteracji na podstawie realnych false positive/negative (osobny projekt treningowy, nie ten). STT/TTS wciąż `MockSTTProvider`/`MockTTSProvider` — bez żadnej chmury, wybór konkretnego dostawcy to następny, jeszcze nieodłożony krok.
3. **Fizyczni klienci satelit (ESP32/desktop)**: Klient desktopowy (Windows/Linux, `services/desktop_satellite/`, sekcja 3.7) **istnieje od poprzedniej sesji** — pełny cykl audio (mikrofon+głośnik) przez `sounddevice`, lokalny VAD końca wypowiedzi, lokalnie syntezowane tony wake/stop, auto-discovery serwera. Wake-word wykrywa dziś realny model (punkt 2 wyżej); STT/TTS nadal Mock. Firmware ESP32 (I2S mikrofon/głośnik, lokalne tony wake/stop) nadal nie istnieje. Web UI pozostaje jedynym zawsze dostępnym nadawcą tekstowym: generuje i trwale zapisuje własny opaque `sender_id` w `localStorage` (`web/js/sender_id.js`) i wysyła go z każdym `POST /api/v1/chat*`, a zakładka "Świat" pozwala zarejestrować tę przeglądarkę (albo dowolny inny `sender_id`, w tym satelitę desktopową) pod pokojem.
