...
- **`vad_silence_duration_ms`** / **`vad_amplitude_threshold`**: Parametry lokalnej analizy amplitudy satelity — domyślnie `1500.0`/`500`. Po wake-wordzie VAD używa obu do wykrycia końca wypowiedzi. Przed wake-wordem ten sam próg amplitudy steruje bramką transmisji: cisza zostaje na satelicie, a po pojawieniu się głosu wysyłane jest również około 1,7 s bufora wstecznego. Każda nowa porcja zaczyna się ramką `SatelliteMessageType.WAKE_STREAM_START`, która zeruje stary bufor modelu na serwerze; zapobiega to sklejaniu dźwięków oddalonych w czasie. Próg jest wysyłany satelicie raz po handshake (`ServerMessageType.CLIENT_CONFIG`) — zmiana działa po następnym reconnect, bez restartu serwera. Konfigurowalne przez `GET/PUT /api/v1/voice/client-config` (zakładka **Klienci**). `vad_amplitude_threshold` ma ponadto serwerowe zastosowanie: `VoiceSession.handle_utterance_end()` odrzuca przed STT nagranie, którego szczyt nigdy nie przekroczył progu, żeby Whisper/Groq nie halucynował tekstu z ciszy.
